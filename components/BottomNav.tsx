...
const NavItem: React.FC<{
  ...
  return (
    ...
    >
      <Icon className={`h-6 w-6 mb-1 ${isActive ? 'text-[#eec35a]' : 'text-gray-400'}`} />
      <span className={`text-xs font-medium ${isActive ? 'text-[#eec35a]' : 'text-gray-400'}`}>
        {page}
      </span>
    </button>
  );
...
const BottomNav: React.FC<BottomNavProps> = ({ activePage, setActivePage }) => {
  return (
    <div className="fixed bottom-0 left-0 right-0 h-16 bg-[#1a4538] shadow-lg border-t border-gray-700/50">
