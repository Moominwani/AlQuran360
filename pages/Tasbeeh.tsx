...
const DhikrButton: React.FC<{ text: string, count: string, onClick: () => void }> = ({ text, count, onClick }) => (
    <button onClick={onClick} className="w-full flex justify-between items-center py-2 text-left text-secondary hover:text-primary transition">
        <span>{text}</span>
...
const Tasbeeh: React.FC = () => {
    ...
    return (
        <div className="p-4 flex flex-col items-center">
            ...
            <div className="w-full max-w-sm bg-secondary rounded-2xl p-6 text-center my-4">
                <div className="relative w-48 h-48 mx-auto flex items-center justify-center mb-4">
                    ...
                    <div className="z-10">
                        ...
                        <p className="text-secondary">of {target}</p>
                    </div>
                </div>
                <button 
                    onClick={handleIncrement}
                    className="w-20 h-20 accent-bg text-inverted rounded-full flex items-center justify-center text-4xl font-bold shadow-lg transform active:scale-95 transition-transform"
                >
                    +
                </button>
            </div>
            ...
            <div className="w-full max-w-sm grid grid-cols-2 gap-4 text-center my-4">
                <div className="bg-secondary p-4 rounded-xl">
                    <p className="text-sm text-secondary">Total Count</p>
                ...
                <div className="grid grid-cols-3 gap-2 mb-2">
                    {[33, 99, 100].map(t => (
                        <button 
                            ...
                            className={`py-2 rounded-lg font-semibold transition ${target === t ? 'bg-green-500 text-white' : 'bg-secondary text-primary'}`}
                        >
                            {t}
                        </button>
                    ))}
            ...
            <div className="w-full max-w-sm bg-secondary p-4 rounded-xl my-4">
